  ...
  const handleExport = () => {
    ...
    if (commissions && commissions.length > 0 && selectedMonthLabel) {
      ...
      if (notebookComms.length > 0) {
        doc.setFontSize(16);
        doc.setTextColor(primaryColorRGB[0], primaryColorRGB[1], primaryColorRGB[2]);
        doc.text('Serviços de Notebook', pageMargin, yPos);
        yPos += 8;
        ...
        const subNotebook = notebookComms.reduce((acc, curr) => acc + curr.commissionAmount, 0);
        doc.setFontSize(11);
        doc.setTextColor(50);
        doc.setFont('helvetica', 'bold');
        doc.text(`Total Comissão Notebook: R$ ${subNotebook.toFixed(2)}`, pageMargin, yPos);
        yPos += 15;
      }
