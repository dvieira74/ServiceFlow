...
const addCommissionSchema = z.object({
  ...
  serviceType: z.enum(['printer', 'toner'], { required_error: "Tipo de serviço é obrigatório."}),
  ...
  if (data.serviceType === 'printer' && (!data.printerModel || data.printerModel.trim() === "")) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Modelo da impressora é obrigatório para o tipo de serviço 'Impressora'.",
      path: ["printerModel"],
    });
  }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          {watchedServiceType === 'printer' && (
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="printerModel" className="text-right">Modelo Impressora</Label>
              <Input id="printerModel" {...form.register("printerModel")} className="col-span-3" placeholder="Ex: HP LaserJet M125a" />
            </div>
          )}
